...
T.CarTrack.prototype = {
    ...
    update: function () {
        ...
        if (this.options.speed > 0) {
            //计算小车所在的像素点
            var l = linePath.node() ? linePath.node().getTotalLength() : 0;
            var s = (this.options.Counter - 1 ) / this.options.nodeslength
            var l1 = s * l;
            var p1 = linePath.node() ? linePath.node().getPointAtLength(l1) : 0;
        ...
        //车辆偏转角计算。
        if (this.options.Counter > 1 && !this.options.notRotate) {
    ...
    start: function () {
        ...
        this.state = 1;
        if (this.D3OverLayer && !this._timer && this.options.interval > 0) {
